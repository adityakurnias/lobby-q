  ...
  <div class="min-h-[calc(100vh-120px)] flex items-center justify-center px-6 py-12 relative font-mono">
    <div class="w-full max-w-md relative z-10">
      <div class="text-center mb-8">
        ...
        <h1 class="text-xl font-black text-retro mb-2 font-display uppercase tracking-wider">Masuk ke LobbyQ</h1>
        <p class="text-[var(--color-text-tertiary)] text-[9px] font-bold uppercase tracking-widest">Sistem manajemen antrean retro-tech.</p>
      </div>
      ...
      <p class="text-center text-[10px] text-[var(--color-text-secondary)] mt-6 uppercase font-bold tracking-wider">
        Belum punya akun?
        <NuxtLink to="/register" class="text-primary hover:underline font-extrabold">
          Daftar sekarang
        </NuxtLink>
      </p>
    </div>
...
const handleLogin = async () => {
  ...
  try {
    ...
    errorMsg.value = err?.data?.message || 'Email atau password salah.'
  } finally {
    loading.value = false
  }
}
